...
if (isset($_GET['search'])) {
    $total_registros_marcas = $subCategoryManager->getSubategorySearchedLength($_GET['search']);
} else {
    $total_registros_marcas = $subCategoryManager->getSubCategoryLength();
}
    ...
    <div class="content-container">
        ...
        <div class="content-breadcrumb-box">
            <div class="breadcrumbs">
                <p><a href="http://localhost/urban/backoffice/">Dashboard</a> / <a href="http://localhost/urban/backoffice/catalogo/categorias/">Categorías</a> / Subcategorías</p>
            </div>
        </div>

        <div class="content-header">
            <h1 class="content-title">Subcategorías</h1>
        </div>

        <div class="content-description">
            <p>Las subcategorías son los conjuntos de productos que se encuentran en la web. Los clientes potenciales harán búsquedas en base a las subcategorías que ellos estén interesados en comprar.</p>
        </div>

        <div class="content-menu">
            <div class="content-menu-box">
                <ul>
                    <li><a href="http://localhost/urban/backoffice/catalogo/categorias/subcategorias/" class="active-item">Overview</a></li>
            ...
            <div class="content-search-box">
                <form class="form-search" id="form-search">
                    <div class="form-group" data-required="true" data-type="search">
                        ...
                        <input type="text" class="form-input" name="search" placeholder="Busca por el nombre de la subcategoría" autocomplete="off">
        ...
        <div class="table-container">
            <table class="table-element">
                    ...
                    <tr>
                        ...
                        <th>
                            <button>
                                <a href="
                                <?php
                                    echo 'http://localhost' . htmlspecialchars($_SERVER['PHP_SELF']) . '?page=' . $pagina_actual . '&sort=' . $sort . '&order=nombre_cat';
                                    ...
                                    ?>
                                ">
                                    Categoría padre <span><i class="bi bi-caret-down-fill"></i></span>
                                </a>
                            </button>
                        </th>
                ...
                <tbody class="table-body">
                    ...
                    if (isset($_GET['search'])) {
                        $totalsubcategorias = $subCategoryManager -> getSubcategoriesSearched($order, $sort, $inicio, $registros_por_pagina, $_GET['search']);
                    } else {
                        $totalsubcategorias = $subCategoryManager -> getSubcategories($order, $sort, $inicio, $registros_por_pagina);
                    }

                    for ($i = 0; $i < count($totalsubcategorias); $i++) {
                        echo "<tr>
                            <td class='centered'>" . $totalsubcategorias[$i]['id'] . "</td>
                            <td>" . $totalsubcategorias[$i]['nombre'] . "</td>
                            <td class='centered'>" . $totalsubcategorias[$i]['totalProductos'] . "</td>
                            <td class='centered'>" . convertirEstado($totalsubcategorias[$i]['estado']) . "</td>
                            <td>" . $totalsubcategorias[$i]['nombre_cat'] . "</td>
                            <td class='centered'>
                                <button class='table-btn-edit'>
                                    <a href='http://localhost/urban/backoffice/catalogo/categorias/subcategorias/editar.php?id=" . $totalsubcategorias[$i]['id'] . "'>
