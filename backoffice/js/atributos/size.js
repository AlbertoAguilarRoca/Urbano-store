...
async function sendDataColor(formData) {
    ...
    const resp = await fetch(`http://localhost/urban/backoffice/controllers/SizeController.php?id=${id}`, {
        method: 'POST',
        body: formData,
    });
...
if(formDelete) {
    formDelete.addEventListener('submit', async (e) => {
        ...
        if (formValidation(formDelete)) {
            ...
            if (data === true) {
                window.location = 'http://localhost/urban/backoffice/catalogo/atributos/tallas/';
